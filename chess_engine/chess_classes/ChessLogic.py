...
from rotemai import chess_ai, game
...
class ChessGame:
    ...
    def move_piece_select_target(self, user, x, y):
        print 'ChessGame.move_piece_select_target - user: %s x: %s y: %s ' % (user, x, y)
        
        # check if game accepts selection of piece
        current_waited_state = self.game_data.get_data('token/step/name')
        if current_waited_state != 'waitCellTarget': #  WTF is this for??
        ...
        # preparation of additional placement information
        #   - when pawn move + 2 : memorize en-passant (on him on or on his trace-cell ?)
        enpassant_set = None
        ...
        #  establish the context after moving
        #   - ep case
        ep = False
        enpassed_piece = None
        ...
        # position the moved piece on the target
        ...
        # update the data context and the grid
        data = {
            'line': y,
            'column': x
        }
        ...
        if promo:
            ...
            move_data = {
                ...
                'rook': rook,
                'ep': ep,
                'promo': None,  # impossible to promote outside promote_piece case
        ...
        return True  # end of move_piece_select_target
    ...
    #  promotions is when the pawn becomes a queen
    ...
    def _check_king_troubles(self, side_name):
        ...
        # Check for checkmate
        fen = self._board_array_to_fen(side='other')
        g = game.Game(fen)
        if g.status == 2:
            print ' CHECKMATE \n%s' %fen
        ...
        if self._if_eaten_piece_was('K'):
            print '_check_king_troubles  recognized the checkmate'
            return True, 'checkmate'
    ...
    def _finalize_turn(self, move_data):
        ...
        king_checks = self._check_king_troubles(enemy_side)
        ...
        self.game_data.set_data('token/step/side', enemy_side)
    ...
    # when opponent is the machine (-1)
    def rage_against_the_machine(self):
        print 'ChessGame.rage_against_the_machine'
        
        fen = self._board_array_to_fen()
        g = game.Game(fen)
        ai = chess_ai.AI(g, 3)

        user = 'ChessBot'  # very dangerous!!! user should come from the db.

        # ----  AI  ----
        res = ai.ab_make_move(fen)
        print " results from ai: %s user: %s" % (res, user)
        ...
        # ----  AI makes a move (in db)  ----
        self.move_piece_select_source(user, fromX, fromY)
        self.move_piece_select_target(user, toX, toY)
        
        return True
      
    def _board_array_to_fen(self, side=''):
        board = self.game_data.get_data('board')
        fen = ''
        ...
        for row in range(8, 0, -1):
            ...
            for c in range(97, 105):
                ...
                if cell == '-':
                    ...
                    pawn = cell["r"]
                    if pawn.lower() == 'h':  # damn french!!!
                        pawn = 'n'
                    fen += pawn.lower() if cell["s"] == 'b' else pawn.upper()
    
            # add position to fen string (end of new row)
            if empty_cells > 0:
                fen += str(empty_cells)
            if row > 1:
                fen += '/'
        ...
        sidedb = self.game_data.get_data('token/step/side')
        if side == '':
            fen += ' w ' if sidedb == 'white' else ' b '
        else:
            fen += ' w ' if sidedb == 'black' else ' b '

        #  fen - castling
        bq = bk = wq = wk = ''
        white_castle = self.game_data.get_data('token/step/castle/white')
        black_castle = self.game_data.get_data('token/step/castle/black')
        
        if not white_castle == '-':  # not empty
            wq = 'Q' if white_castle and 'r1' in white_castle else ''
            wk = 'K' if white_castle and 'r2' in white_castle else ''
          
        if not black_castle == '-':  # not empty
            bq = 'q' if black_castle and 'r1' in black_castle else ''
            bk = 'k' if black_castle and 'r2' in black_castle else ''
        ...
        step = self.game_data.get_data('token/step')
        
        if 'enpassant' in step:
            x = step['enpassant']['x']
            y = step['enpassant']['y']
            ycoords = y
            xcoords = int(x) + 97
        ...
        # 0 Halfmove clock & Fullmove number:
        # 1 Halfmove clock: This is the number of halfmoves since the last capture or pawn advance. This is used to
