...
class CreateChessGameForm(forms.Form):
    ...
    name = forms.CharField(label='Game name', max_length=200, validators=[RegexValidator(regex='^[\w _-]+$', message='Invalid Game Name')])
    ...
    aibot = forms.BooleanField(label='AI Chess Bot', required=False)
    ...
    def execute(self):
        try:
            ...
            aibot = self.cleaned_data['aibot']

            game = GamePersistentData()  # adds logging info from models
            ...
            # game.set_data('game_options/aibot', aibot)
            ...
            if aibot:
                play_against = 'black' if play_as == 'white' else 'white'  # in case player decided to be black
                # ROTEM TODO - make sure to update -1 as machine player (aka - don't update the machine user)
                game.set_data('participants/%s/1' % play_against, -1)
            game.save()
